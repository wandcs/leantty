...
- [ ] 从未授权状态执行首条 `put/get`：只出现真实系统 Downloads 权限弹窗；拒绝后终端仍可用，
  ...
  隔离应用身份或明确可清理的设备状态上完成。2026-08-14 又实测 DevEco 本地 MateBook Pro
  虚拟机：系统为 API 24，但设备 ABI 只有 `x86_64`；同一保留候选
  `0116b6ecf02f7501541acf7ef654beb4c57246833a28d0de7a8f7240e7826c99` 在安装阶段即被系统以
  `9568347` 拒绝，明确报告设备 ABI 与模块原生库不匹配。虚拟机因此不能运行生产
  `put/get -> Downloads` 事件链，也不能闭合本项；不得为这一个权限前置条件增加不受支持的
  x86_64 产品构建或第二套测试专用授权路径。剩余有效前置条件是可安全重置的物理 ARM64 PC
  或等价的隔离 ARM64 应用身份。
