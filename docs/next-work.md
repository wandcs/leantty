...
**当前执行位置：1. Mosh 跨阶段输入守卫修复已验证，旧证据继承边界已核定；保留原候选，下轮重新 QH 后按 R3 验收，不自动启动人工全矩阵。**
...
- [ ] 新干净 harness 完成 QH 后，用原候选按 R3 完成正式验收。
  维护者于 2026-09-12 要求先给出收敛方案，不自动启动需要其配合的完整矩阵。
  Mosh 跨阶段守卫的最小反例证实重排误拒绝与同路径替换 Web 误放行。现保留本次焦点
  捕获的 node/layout，输入前、重试前及 Enter 前统一使用共享原生 owner 判定。26 项
  真实调用反例、七项组合软件检查、原 HAP 固定端点诊断及独立收尾审计通过。
  本轮工具修复与审计记录见 [PR #181](https://github.com/wandcs/leantty/pull/181)。
  旧八场景的最后输入布局对均通过新检查，但此前命令的布局对被覆盖，不能重核全部
  历史输入。保留 C0–C2 和旧恢复事实；整轮 C3 不满足跨 harness 继承前提，采用 R3。
  不放宽哈希条件，不引入无法补回历史数据的继承机制，不用诊断补写正式 Pass。
  详见 [修复与证据取舍](design/agent-tui-compatibility.md#2026-09-12-mosh-命令跨阶段-owner-与旧证据审计)。
