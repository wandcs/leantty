...
**当前执行位置：1. PR #187 已合并，Pi/tmux 的已知限制不再阻断其他验收。Qwen 通知前置诊断发现夹具先隐藏、后启动会丢失首次失焦；启动顺序修理及一次零模型真机焦点/退出/清理验证通过。下一步核对最小化到主机观察隐藏之间的通知时序判据，再冻结工具并按 R2 继续 Agent/SSH。未改产品、原 HAP 或历史失败报告；C3/C4 未闭合。**
...
- [ ] 先核对最小化事件到主机读到隐藏日志之间的通知观察判据。Qwen 已改为前台启动、真实 1004 就绪后再最小化；公开 hook 八项检查、真实 tmux 对照、28 项 attention 反例和一次零模型真机焦点/退出/清理均通过，但这不证明原生通知。用输出检查点与设备状态/发布顺序区分隐藏前、转场内、隐藏后及未知情况；先验证因果门槛，不调用模型寻找一次响铃、不按 Agent 名称追补豁免。必要的最小观察器修理验证后，再冻结工具、刷新准入与 QH，执行 R2 Agent/SSH 后缀。Pi/tmux 保留已证实的 `upstream-not-forwarded`，产品、隐私、交互、未知和清理失败仍阻塞。详见 [焦点启动修理](design/agent-exit-boundary-20260912.md#qwen-focus-readiness-repair)。
