...
**当前执行位置：维护者于 2026-09-13 确认指纹持久化修复。串行异步提交、等待反馈及
取消/销毁隔离已实现，针对性软件、SSH/Mosh 真机验证和独立清理通过。产品树已
改变，下一步按 R4 建立新候选，不再按旧 R2 计划复用原包行为证据；C3/C4 未闭合。**

实现、原包对照、包体身份及验证限制见
[指纹持久化修复](design/trust-persistence-latency-20260913.md)。维护者休息期间优先完成
自动工作；WSL 生命周期仍由维护者管理，需要真实锁屏/合盖配合时暂停，不播放提醒。

- [ ] 按 R4 从已合并干净来源准备新候选，完成正式软件门、QH 和完整适用 C3；不使用
  原 `ac01156` 产品树的行为报告拼接新候选通过。固定模型预算，首错停止并独立清理。
