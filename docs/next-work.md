...
**当前执行位置：1. PR #188 的焦点启动修理已合并；通知观察判据已改为外层原生帧与设备状态/发布顺序共同证明，实际卡片及同一 Pane 返回子链得到真机验证。诊断脚本的元数据误判导致整次结果仍为中断，单次独立恢复及清理审计通过，不改写原报告。下一步冻结修理、刷新 R2 准入和 QH，再继续 Agent/SSH。未改产品或原 HAP，零模型请求；C3/C4 未闭合。**
...
- [ ] 以已冻结的通知时序修理刷新 R2 准入与正式 QH，再执行 Agent/SSH 后缀。要求原 HAP 不变、当前临时资源缺席和新 QH 通过；本次诊断的两份失败报告不作为正式证据复用。最小化检查点不再冒充设备事件时刻：原生外层帧、当前进程隐藏→attention→发布顺序、通用卡片及同一完整 Pane 返回共同判定。Pi/tmux 保留已证实的 `upstream-not-forwarded`；不得按 Agent 名称新增豁免，产品、隐私、交互、未知和清理失败仍阻塞。详见 [通知观察时序修理](design/agent-notification-order-20260912.md)。
