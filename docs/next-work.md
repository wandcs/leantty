...
**当前执行位置：1. Agent 退出边界修复及八项零模型真机诊断通过；接下来建立可信的最小正式续跑，保留原候选，不自动重跑人工矩阵。**
...
- [ ] 核定并建立可信的最小 R2 续跑：新 QH → Agent 正式矩阵 → SSH 正式矩阵。
  ...
  先解决正式汇总入口对跨 harness 身份及原 cleanup failed 的继承限制：新记录只引用
  已验证的原始报告、独立恢复审计、精确候选和场景局部变更，不改写旧结果、不手工拼接
  Pass。验证初始状态与前缀独立性、完成新 QH 后，再运行未完成的 Agent 和 SSH 门禁。
