...
**当前执行位置：1. 通知验收工具已通过定向验证；先核定新发现的图标工具依赖告警，再冻结正式复验范围。**
...
- [ ] 先核定并处置 GitHub 新提示的 [sharp 高危告警](https://github.com/lovell/sharp/security/advisories/GHSA-rgj7-g3m4-5g8c)。
  `tools/package-lock.json` 固定 0.35.3，上游修复版本为 0.35.4；本地唯一代码消费者是
  `generate_icons.cjs`，输入为仓库固定 SVG，尚未发现应用运行时调用或不可信 HEIF 输入。
  它不同于正式 Web 构建使用的 `tools/web-terminal` 依赖。本轮只读核对并记录，不升级、
  不关闭告警；先明确升级/处置与冻结候选的关系，避免刚跑完矩阵又因输入变化重跑。

- [ ] 告警处置和候选边界明确后，固定兼容的干净 harness，完成 QH 与正式物理矩阵。
  仅本次通知工具修复的范围是保留原候选并 **R3**，不是自动重建应用。
