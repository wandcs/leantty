...
当前停点（2026-09-13）：SSH 失败诊断的跨回调顺序依赖已修复，并通过 15 项定向软件
检查、ARM64 构建、同包 SSH 诊断与 Mosh 固定端点验证及清理。失败状态/reason 与
Session 结束由同一控制事件承载，不加延时、重试或过期事件缓存。真机观察包未复现
旧乱序；根因边界、真实所有者反例与验证限制见
[失败诊断合同](design/ssh-diagnostics.md#2026-09-13-失败诊断与-session-结束的原子事件)。

下一步从合并后的远程来源按 R4 建立新精确候选，再执行正式自动段。产品/native 已变，
不得沿用工具 R2 将旧 C3 提升为新候选通过；旧 Agent/前缀只保留原身份，不追加模型探索。
自动段清理并到达 awaiting-operator 后停止，维护者明确就绪才继续锁屏/合盖。
本轮零模型、无人工动作或语音；完整第一段和 C4 尚未完成。
...
**当前执行位置：SSH 产品修复的开发验证已闭合，正式验收须从新 R4 候选重新开始。
旧 `a5d637d` / `e68758b5…7bfef` 候选与 PR202 结果保持原身份，不改写原失败。**

实现、原包对照、包体身份及验证限制见
[指纹持久化修复](design/trust-persistence-latency-20260913.md)。维护者休息期间优先完成
自动工作；WSL 生命周期仍由维护者管理，需要真实锁屏/合盖配合时暂停，不播放提醒。

- [ ] 建立修复后的 R4 候选，完成自动段及清理后暂停，再由维护者明确就绪续接两项人工。
  不扩充跨工具版本续验框架或兼容白名单，不增加模型种类或自动重试，不改写旧报告。
