...
**当前执行位置：1. PR192 正式续验在 OpenCode 输入前焦点守卫停止。Agent 已获焦时不再额外点击，必要点击后验证当前所有者，并保留安全失败元数据；110 项实际函数回归、定向软件检查及原 HAP 的零模型输入对照通过。旧／新方式都通过，历史焦点丢失原因仍未知，不追加诊断采样。下一步冻结修正、重审 R2 准入，再做新 QH 和完整 Agent/SSH 后缀；C3/C4 未闭合。**
...
- [ ] 冻结已验证的 Agent 焦点准备修正后，重审 R2 准入并完成新 QH 和完整 Agent/SSH 后缀。原 HAP、所有历史报告与冻结来源必须不变；将 PR192 正式失败及本轮两次零模型诊断纳入资源审计，诊断结果不复用为正式证据。已确认并修复的是冗余点击、点击前旧布局返回和安全失败信息丢失；旧／新真机对照均通过，不能据此宣布历史焦点丢失已修复。停止追加因果采样，不改产品或共享守卫；正式续验使用固定预算，首错停止并保留阶段、焦点数量和所有者比较信息，再决定诊断。通知的外层原生帧、设备顺序、通用卡片/隐私及同一 Pane 返回判据不变。Pi/tmux 只保留已证实的 `upstream-not-forwarded`，未知、输入或清理失败仍阻塞。详见 [焦点修正、对照与重审结论](design/agent-notification-order-20260912.md#follow-up-observe-agent-input-focus-without-a-redundant-click)。
