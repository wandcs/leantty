...
**当前执行位置：1. Mosh 0.1.1 与验收观察器修复已通过定向验证；从本批次合并后的干净来源建立 R4 候选。**
...
- [ ] 从本修复批次合并后的干净 main 固定新发布来源，完成 R4 候选与正式验收。
  依赖/打包输入改变必须 **R4**，建立新精确候选，不续接旧正式前缀。
  保持 Mosh 八场景顺序，全部适用场景及清理通过前不声明完整 C3，不进入 C4。
  正式合盖若保留原进程与 Session，必须验证同一 PTY 执行新命令；若走工作区恢复，
  如实记录该分支和同一 Session 错误恢复未覆盖项，不以工作区恢复替代 PTY 存活证据。
  本轮诊断通过也不替代正式验收。不直接修改上游或重开后台长时任务。
