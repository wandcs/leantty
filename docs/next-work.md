...
**当前执行位置：新 R4 候选的 C1/C2 已完成，保留 `a5d637d` 来源与
`e68758b5…7bfef` HAP。PR196 分段入口和 PR197 Host 清理等待已合并；自动 R3
在 Qwen 直连通知过渡区间及失败收尾处停止，原报告不改写。窗口时序、失败清理和
任务完成门闩的零模型诊断已闭合；原生 Qwen 及正式 C3/C4 仍待验证。**

本轮只修理验收工具：Qwen 使用官方任务完成提醒，待真实隐藏、原生失焦和执行时长
达标后完成受控任务；失败后只在确认的本地测试 Tab 删除临时指纹。115 项所有者检查、
定向软件门和独立资源审计通过。任务门闩真机通知子链通过，但诊断适配器作用域导致
收尾报告失败，原报告保持失败；不重复物理采样或补写 Pass。详见
[证据和复验范围](design/agent-notification-order-20260912.md#qwen-task-completion-precondition-and-failed-check-cleanup-2026-09-13)。

实现、原包对照、包体身份及验证限制见
[指纹持久化修复](design/trust-persistence-latency-20260913.md)。维护者休息期间优先完成
自动工作；WSL 生命周期仍由维护者管理，需要真实锁屏/合盖配合时暂停，不播放提醒。

- [ ] PR198/199 已合并。PR199 自动轮六阶段通过，15:37 在默认 ECDSA 的权限准备
  阶段停止：全屏布局混入桌面的同名应用卡，导致唯一匹配失败；没有导出或删除原密钥，
  cleanup passed，模型工作负载和人工操作均为零，原失败报告保留。工具仅将 Downloads
  布局限定到系统设置包；36 项定向检查和一次真机权限往返／恢复通过。合并并冻结修复后，
  保留同一 C0–C2，按 R3 更新 QH 并执行自动段；不增加跨工具版本续验框架。预算为九个
  固定模型工作负载，零自动重试。完成独立清理并取得 `awaiting-operator` 后停下；
  维护者明确说可以开始后，仅续接真实锁屏和合盖两项。第一段不播放提醒、不给旧报告补通过。
- [ ] 异常退出恢复提示后不能拖动窗口外边框的反馈保留待复现。原包单 Pane 的正常/
  异常恢复对照均能拖动缩放；只有一组样本，不作已修复结论。等待原现场窗口模式、
  边缘和输入设备条件，不重复强杀或添加窗口 workaround。
...
3. 诊断已按固定预算结束：checkpoint 仅 3 个有效样本，hitch unavailable，长期泄漏、系统
   OOM 和同系统 1.5.1 对比未证明。除已获准的指纹异步修复外，保留其他同步持久化、
   warm/队列预算和既有 xterm 补丁；不按
...
以下为旧候选的停止与工具修复历史，不再授权执行旧 R2 续跑。PR193 已冻结 Agent 焦点
修正，其正式续验随后因测试机充电离线停止；原报告仍为失败，SSH/C4 未开始。设备重连
后已独立清理该轮自有资源。历史焦点丢失和 Codex 通知原因仍未知；不追加因果采样，
不把诊断或第三方原因改写成通过。Pi/tmux 只保留已证实的 `upstream-not-forwarded`，
未知、输入或清理失败仍阻塞。旧 HAP、报告和冻结来源保持不变。
见 [焦点修正与证据](design/agent-notification-order-20260912.md#follow-up-observe-agent-input-focus-without-a-redundant-click)。
