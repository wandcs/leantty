...
>
> 当前阶段：1.3 功能与人工审查已闭合；下一步收口 release source，并从精确提交冻结正式候选
>
...
1. [x] 按 Tab 补全设计修复 `put/get` 的本地候选区与交互状态。候选区必须在当前命令下方且同一
   ...
   高亮共享同一当前项。候选一旦进入选择态并写入命令行，就是当前真实输入；普通文字、空格、
   Backspace/Delete 必须保留该候选、关闭候选区，再执行普通编辑，只有 Esc 才恢复进入选择前的
   输入。后续补全由用户再次按 Tab 发起，不维持实时筛选菜单。Tab/Shift+Tab 与四方向键按二维
   列表导航；Enter 只接受候选；目录保留 `/`，重复输入已经自动附加的 `/` 只保留一个且关闭菜单，
   不再隐式打开下一层。保持已有 Downloads 边界、无联网、无递归、无权限弹窗、
   100 候选上限、控制字符净化和固定字体 cell 宽度规则，不引入 Shell、模糊搜索、图标、设置或
   ArkUI 文件面板。
   完成条件是自动化覆盖关闭/列表/选择、无/一/多候选、选中后的空格/普通输入/Backspace/Delete、
   同区清除、二维导航、Enter/Esc、重复 `/`、候选溢出、Unicode/宽字符、终端换行与 resize，并证明无网络、认证、
   ...
   2026-08-16 的实际使用推翻了“选择项只是视觉预览”的合同：高亮文件后按空格会先回退到 Tab
   前的前缀，导致文件名消失；普通文字、Backspace 和 Delete 共享同一错误路径，`/` 只是绕过它的
   独立特例。用户批准按成熟 Shell 的简单主路径重开本项：已选候选成为真实命令行，普通编辑关闭
   菜单并作用于当前文本，需要继续补全时再次按 Tab；旧 HAP 与 112/112 结果不证明修订后的合同。
   修订实现将旧 `PREVIEWED/baseInput` 语义收敛为 `SELECTED/rollbackInput`：选择项进入真实命令行，
   仅 Esc 使用回滚快照；普通文字、空格、Backspace/Delete 结束补全后编辑当前文本，不再重新枚举，
   重复目录 `/` 只去重并关闭菜单。ArkTS 114/114、验收源码、构建工作流、离线指南和公开源码检查
   通过，ARM64 debug HAP 构建、签名、安装和启动通过。首轮真机完整矩阵在所有新增路径通过后，
   于隐藏文件用例发生 HDC 文本注入丢失一个 `n`；不修改生产代码，完整复跑随后通过。最终证据位于
   `build/verification/tab-completion-selected-editing-rerun-20260816/`，验收 HAP SHA-256 为
   `9e490794b665d61073f29e0fe52a2c15d0c1f57a3ff45144527fc5096ce7436a`；实体空格、Backspace、
   Enter/Esc、二维导航、重复 `/` 后再次 Tab、空格/引号/Unicode/隐藏项和控制字符过滤均使用生产
   命令缓冲区判定，且没有权限、认证、网络或传输副作用。
2. [x] 把“每次重新生成帮助 HTML 后，必须对中文做一次说人话复核”固化到离线指南设计与发布
   ...
   `354a813fb9fb2a717153293833aba90ef2308c9245757ddd8712cd30b7a2d6d5`，指南自动化通过
   （70,561 bytes、3 套 code palette）。本机预览件位于
   `build/design-review/LeanTTY-User-Guide-preview.html`；内置浏览器安全策略允许用户查看，
   但禁止代理读取该 `file://` 页面的 DOM 或截图。随后把 Tab 补全改为醒目的三点中英文提示，
   审查源、打包资源和预览件 SHA-256 均为
   `9d6710b51a6744a23c0d8eea8a817eb0f1255ffbdb660a361b35109daa34c337`，指南自动化通过
   （70,658 bytes、3 套 code palette）。2026-08-16，用户确认整份帮助页人工审查和测试结束，
   本项关闭；后续每次重新生成仍须重新执行同一中文复核。
...
- [x] 将用户可见改动记录到 `CHANGELOG.md`。在功能和依赖稳定、准备保留候选时再按
  ...
  已发布能力。
- [ ] 收口 1.3 release source：把当前已验证的 Tab 补全编辑合同、帮助页收尾和验收脚本改动作为
  聚焦提交合入并推送 `main`；
