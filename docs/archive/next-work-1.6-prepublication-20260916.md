...
2026-09-16 发布状态收尾时保存。仅用于追溯；不授权执行，当前 TODO 只见 docs/next-work.md。

# LeanTTY 当前工作

当前停点（2026-09-13）：PR204 的新候选已通过 C1 29 项和 C2，HAP SHA-256 为
`d1133767247a932bbe7d669eeb6e8bcc64f5c3adcf1508be63ca05485e965d7c`。自动段十六项通过，
Agent 在 opencode/direct 通知证据缺口处停止。日志有 attention 与发布，但未保留当时
专用查询的解析输入，卡片/返回未验证；清理及独立资源审计通过，旧报告保持失败。

当前有界工具工作：只保存失败时最后一次 owner 查询、解析结果和步骤，不改通知判定、
查询标签/窗口、产品日志或 HAP，不在等待期间写文件。预计 20–30 分钟，实际函数先红
后绿，覆盖查询失败、截断、错误归属及异常收尾。若需要跨快照状态机或新的观察层，先
重定。一次零模型对照已证明专用查询仍可保留隐藏事件，未复现原缺口，不再重复。
原 opencode/direct 的一次定向诊断已通过：原生 OSC 99、完整 owner 事件链、通用卡片/
隐私、原 Pane 返回、交互和清理均成立。计划一个工作负载、零自动重试，实际模型请求
总量不可用。历史缺口未复现，不声称修复了未证明的产品时序问题；已修复的是失败证据
不可追溯。下一步合并此局部工具修理，重新准入/QH，按已有 R2 复用十五项独立行为
前缀，重做完整 Agent 八组并续接 SSH/自动 Mosh；不复用 Agent 内单项诊断或扩大白名单。
第一段尚未完成，不进入锁屏/合盖或 C4，不将诊断提升为正式通过。
...
**当前执行位置：新候选已建立，先关闭 Agent 通知证据缺口再续接自动段。
旧候选与失败报告保持原身份，不重建当前 HAP，不改写原失败。**

实现、原包对照、包体身份及验证限制见
[指纹持久化修复](design/trust-persistence-latency-20260913.md)。维护者休息期间优先完成
自动工作；WSL 生命周期仍由维护者管理，需要真实锁屏/合盖配合时暂停，不播放提醒。

- [ ] 关闭通知证据缺口，按既有 R2 完成自动段及清理后暂停，维护者明确就绪再接两项人工。
