...
2026-09-16：产品开发、性能/稳定性/代码质量诊断、正式 C3/C4 和 GitHub 1.6.0 发布
已完成，同版本 production APP 与提审材料已交付维护者。2026-09-20 维护者确认
1.6.0 已通过 AppGallery 审核并上架。来源、摘要和限制见 [发布记录](release/1.6.0-status.md)；
后续合同及原型工作仍受前置门约束，活动顺序只在 [Next Work](next-work.md) 中维护。
...
## 开发中 milestone：1.7 — 原生终端与会话边界

2026-09-25：选定产品改动已合入。维护者批准的
[差分验收](design/1.7-differential-acceptance-20260925.md)及 C4 同源签名、归档和短冒烟
已完成；旧完整矩阵仅作为历史基线，新包不声称完整矩阵通过。随后实际使用出现单个
Tab 终端故障，当前暂停发布并定向诊断；尚未创建 1.7.0 tag、发布 GitHub Release 或
提交 AppGallery，具体证据与恢复条件只在 Next Work 维护。

2026-09-23：维护者批准 Known Hosts 改用应用私有文件，升级迁入已提交信任后清退旧
Asset，随后卸载重装不恢复主机信任；其他资产合同不变。维护者明确将这项合同变化
作为狭义版本政策例外纳入未发布的 1.7.0，不改变一般版本规则。旧候选保留原身份，
当时要求以新提交/包重新取得受影响证据，并按 Next Work 暂停正式验收。

2026-09-11：维护者确认两项规划目标。09-16 独立原型已支持技术可行性，受控 GPU 恢复缺口已修复验证；1.7 的正式替换仍以
正确性、恢复能力、用户收益和维护成本证据为进入条件。活动顺序只在
[Next Work](next-work.md) 中维护，既有 MatePad 设备门禁与优先级规则继续适用。

2026-09-17：维护者确认原型调研通过并授权产品开发。按已接受的设计进入最小会话边界与
原生终端集成；原型通过不代替产品恢复、既有交互和同机性能验收。

2026-09-20：开发期完整替换及总审已闭合，debug/release 均为单一原生路径。
范围与证据边界见[迁移审计](native-terminal-migration-1.7.md)；当时尚未合入或通过正式候选验收。

### 用户结果与范围

- 通过官方 Ghostty VT 核心与原生 GPU 改善终端体验，减少 Web 终端桥接和恢复成本。
  最低 API 按维护者已确认的 24 设计；在此基线上保持已有命令、数据、交互和 SSH/Mosh 合同。
- 终端核心与具体会话解耦：同一终端核心和 GPU 表面可由合成数据源、SSH、Mosh 驱动，
  更换来源不修改渲染代码。未来系统本地会话可以沿同一边界接入。
- 保留 Tab/Pane/Session 的唯一所有权，明确终端状态、Surface、`ltty>` 输入与具体会话
  的责任；窗口资源重建不等同于关闭会话，协议结束前已接受的输出仍须按序处理。

### 取舍与进入条件

原生 renderer 接入需要一个清楚的终端交互边界；将两项工作放在同一版本，可以避免把
现有会话耦合带入新 renderer，也不必为旧 Web 实现另做完整架构改写。边界只覆盖现有
SSH/Mosh 的真实交互，保留它们各自的认证、状态同步、预测与恢复语义。

采用官方 Ghostty 上游并自行封装，不复用 `libghostty-ohos`。窗口、输入法、文字等基础
能力优先采用鸿蒙公开接口，只有明确证据证明不足时才引入对应第三方库。GPU 路线和
文字缓存候选的具体边界见 [技术方案](design/ghostty-native-terminal.md)。

本版本不交付本地 shell、系统进程管理、HSL 直接 transport、Linux 兼容层或通用插件框架。
未来系统本地会话的具体接入等待公开能力、权限与真机证据；本轮规划不承诺它已可用。
原型若不能满足正确性、恢复或合理维护成本，应收缩或停止替换，再按证据修订范围。

独立原型结果见[研究记录](design/ghostty-native-terminal.md)，不等于生产集成或完整替换验收。
维护者于 2026-09-20 确认 1.6.0 已过审上架，下一高版本合入的[版本前置条件](versioning.md)
已满足。1.7 仍按代码审查、PR 与正式候选流程推进；开发交付不等于新版本已发布。
