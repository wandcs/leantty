...
**已修复：** PR #256 把每次重新生成的七份通知放到既有
`.cache/native-terminal/licenses`，由 LeanTTY `-Clean` 明确清退；OHPM 安装复用
既有文本比较并恢复锁文件原字节，真正依赖漂移仍拒绝。独立 `p9` 在 `6a5eb82` 的
真正首次 clean 已通过通知、锁文件、完整 manifest 和生产 APP/HAP 签名核验。
证据 `freeze-1.7.0-20260925/c4-r2/result.json`；后续不同失败见下一节。

同日新产品 `5a4f8d6` 的独立 `p8` 首次 clean 再现同一事件链：原生生成后日志出现
`::clean`，打包签名完成时整个 `build/native-terminal` 已不存在，元数据第 569 行失败。
本轮遵守差分方案的失败停止条件，未执行旧轮的恢复重跑；两份锁文件仍只有换行变化，
保存副本后恢复，检出干净。证据 `freeze-1.7.0-20260925/c4.log` 与 `c4-timing.json`；
阻断已进入唯一清单 next-work，不新增平行工具审计。

同一冻结 `06160c0` 生产 clean 构建完成原生编译及签名，Hvigor 随后执行根项目
`clean`，删除先前生成的整个 `build/native-terminal`。元数据读取七份许可证时停止。
首次恢复又被 OHPM 两份锁文件的纯换行差异拦在预检；Git 内容 diff 为空，保留副本并
恢复 HEAD 字节后，以原入口不带 `-Clean` 恢复成功，七份通知和完整签名 manifest
均通过。原始失败与两次恢复日志保留在发布区
`formal-verification/1.7.0-06160c0-r3-20260924/c4-production*.log`。

目录清理与后处理顺序是构建工具的责任，不能省略许可证检查或改写成功标志。早期批次
采用原入口恢复，保持产品提交不变；后续持久修复应把 Hvigor 初始化/清理放在旁路
原生生成之前，或在其清理范围外保存必需通知，并复用既有 OHPM 字节恢复与内容漂移
拒绝机制。验证应覆盖真正首次 clean 构建、七份通知、锁文件字节不变及完整 manifest，
无需为此重复模型或产品真机矩阵。此为后续工具改进记录，不授权扩大当前 C4 工作。

### 2026-09-25：全新评审目录缺少 SDK 模块缓存

**已修复：** PR #258 显式把已验证的生产检出传给签名 bridge，SDK 初始化使用该
上下文，评审目录继续提供测试签名配置和输出。没有缓存搜索、额外编译或回退路径。
冷评审上下文与失败时不泄露凭据的定向回归通过；`m10` 工具固定后，复用 `p9`
unsigned HAP 的真实签名/比较/归档 5.7 秒通过，短 review smoke 23.7 秒且清理通过。
最终证据 `freeze-1.7.0-20260925/c4-r3/differential-acceptance.json`。

上述 `p9/r9` 续轮在生产 manifest 生成后停止于 `sign-review-hap.cjs` 的 SDK 模块加载，
尚未进入解密或签名。SDK `initNodePath` 依据当前目录选择 Hvigor 项目缓存；无凭据的
只读对照确认 `p9` 缓存存在且模块可加载，未编译过的 `r9` 缓存缺失，返回
`MODULE_NOT_FOUND: @ohos/hvigor`。证据为同目录 `module-diagnosis.json`。
这是签名工具对冷启动目录的环境假设缺口，不能归为证书错误或产品失败。发现时按停止
条件保留产物并报告，不自动重试；后续优先明确 SDK 工具上下文，复用 unsigned HAP
续接，无须为模块缓存重编译产品。执行范围只在 next-work 管理。

### 2026-09-25：C4 独立编译的原生身份比较

同一 `06160c0` 的 `p7` / `r7` 构建均成功，但 Rust 库嵌入本地 `ssh-key` 补丁的
不同绝对路径。串行让评审在 `p7` 重编译后，Rust 哈希完全一致；搬移导致 OHPM 类型
包 junction 失效的打包失败由现有正式输入准备入口恢复，未修改产品类型定义。

实际包内的终端库仍不同：生产 `b908ec05…4ee5c8`，评审 `1e457baf…d6acc8`，
ELF `.text` 相差 4 字节。清除搬移前的整个 Zig 生成目录后，评审结果仍相同。
不能仅用原有 manifest 的单个 Rust 库摘要宣称两包原生身份一致；没有把未归因的
编译差异归为纯调试信息，也没有复制生产库或改写清单。达到本批停止条件后恢复
两份检出和链接，保留失败结果，未进入真机冒烟。

证据为发布区 `formal-verification/1.7.0-06160c0-r3-20260924/c4-path-comparison/`。
更直接的验收对象是同一生产 payload 的测试签名包；当前仅提出流程调整建议，
未实现新签名或归档路径。可执行范围与维护者决定仍只记在 next-work。

### 2026-09-23：首次开发构建期间证据目录消失

独立鼠标工作树首次默认 debug `build-all` 未传 `-Clean`。捕获器已预建日志目录并
运行 Hvigor，子进程返回后 `WriteAllText` 却因目录不存在失败；随后发现同一 `build/`
下原始红绿日志、分组报告及 native 宿主产物也消失。具体删除触发源未确认。重建
目录后第二次默认构建通过，未为此修改产品或签名流程。

原日志不能补造：另一个工作树在构建前完成的独立复核保留了测试判据和通过情况，
鼠标源码/测试与复核提交逐文件相同；最终包、设备冒烟和 CI 证据仍完整，因此此项
不阻断本次开发交付，也不触发全门禁重跑。现存交付证据另存系统临时目录；若后续
安排工具维护，应查明新工作树首构建清理范围，并使捕获输出在目录被清理时仍能
保存。此处仅记录非阻断观察，不授权新增工具修复；可执行工作仍只进入 next-work。

### 2026-09-20：正式密钥口令报告截断与候选重建取舍

1.7 首个正式候选 C1 的 33 项检查、C2 和 QH 通过。口令场景的操作与清理通过，但
`Write-BehaviorEvidence` 的 JSON depth 7 将新增命令观察中的 submission 对象写成
类型名。运行中的后续备注场景被停止，原始报告保留，不把截断报告当成完整验收。
独立清理确认本轮临时密钥、信任、映射、夹具不存在，屏幕超时已恢复；模型调用为 0。
本地证据位于发布区 `formal-verification/1.7.0-automatic-20260920/`。

修复复用已有原子 JSON 写入器，使用 depth 12 并拒绝任何深度警告。两个实际报告
函数反例先失败后通过：深层 submission 字段完整往返，超限拒绝且保留上一份完整
报告。没有产品代码或日志内容变化，也没有新增报告框架。

原计划以 R3 复用候选；核对发现五处固定跨工具名单不包含本次两文件。约十分钟重新
准备候选，比扩展这些名单、增加采用验证和长期维护更便宜，因此选择已有 R4 路径。
旧轮 C3 不复用，不为保留一次测试结果扩展工具。唯一可执行工作仍在 next-work。
...
- **09-24 新观察：** 列表开发验证中，307 字符准备读回 306，421 字符准备三次均读回 420；
  均在 Enter 前停止并清空，未提交对应命令。生成后配置改用 233/245 字符的独立命令，
  实际每条一次准备即精确通过，列表/重启/清理随后完成。没有定位长输入末字符缺失的底层原因，
  不将其归为第三方豁免或修复产品输入，也不为只读列表验收扩展工具改造。
  证据 `build/verification/local-command-presentation/setup-inexact.log`、`setup-input.json`；
  一次性脚本先前的 HDC 正斜杠绝对路径失败已用标准 Windows 路径对照定位并修正，未改共享工具。
...
## 2026-09-16：开发流程整改——基线清账与既有续验核对

本批只整理工程来源和验证工具合同，没有产品改动、构建、HDC/WSL 操作、模型请求、
签名或发布；不重新执行 1.6 C3/C4。活动工作仍以 [Next Work](next-work.md) 为准。

### 基线和归属

已刷新远端 main 到 `37e03398130d380f860f64429f0ef4642b18bea3`。根工作区 HEAD 仍为
`e2e3f0f`，与后续独立交付目录不同，不能直接作为最终 1.6 产品或最新验收工具来源。
对 165 个差异相关路径按 Git clean-filtered blob 比较，另保留文件原始 SHA-256：

| 分类 | 数量 | 本批处理 |
| --- | ---: | --- |
| 内容已与 main 一致 | 107 | 已交付，不重复提交 |
| 匹配较早 main 历史 | 24 | 保留根副本，后续使用干净来源 |
| main 有、根目录尚未同步 | 15 | 使用完整 main，不从根覆盖工具 |
| 缺少已合并行的组合副本 | 3 | KeyCommandService 和两份测试入口；不当作新修复 |
| 锁文件末尾换行差异 | 1 | 不作为产品改动 |
| 当前任务的治理、计划与归档 | 5 | 三份治理文件选择性合并；计划与快照保留 |
| 原生原型任务拥有的设计/架构文档 | 5 | 不接管、不随本批提交 |
| 本地历史证据增补 | 5 | 原位置保留，不整包覆盖 main 历史 |

这不是“165 个未完成任务”，也不是运行中 workspace 的永久快照。
忽略的 research、冻结 checkout、签名材料和包体未被移动或删除。
完整逐文件处置在本地 `build/verification/development-workflow-20260916/workspace-dispositions.json`；
本批后续编辑和其他任务的新修改不计入这个初始统计。

已从 main 建立隔离分支 `codex/development-workflow-cleanup`，选择性合入此前批准但
尚未进入 main 的交付效率规则。质量文档保留 main 已新增的 Agent R2、自动/人工分段、
通知观察、退出边界和权限准备说明，没有用旧根副本整文件覆盖。

### 验证和实际文档修理

主线“分段模式不能继承 Agent continuation”的旧句与其上方新增合同及实际实现冲突。
真实入口的离线反例已证明 `agent-split-R2` 合法保留独立前缀，故只将该句改为：
允许已核定的 split R2，不允许 full-mode 直接 Resume 或导入零散 Agent 检查。
不改变身份、清理或正式门禁。

| 验证入口 | 本轮结果 | 边界 |
| --- | --- | --- |
| `test-release-agent-continuation.ps1` | 70 + 71 项通过 | 合法继承；身份、独立性、清理或证据被改时拒绝 |
| `test-release-phases.ps1` | 32 项通过 | 真实编排，外部副作用替身；自动/人工段不重复，失败停止 |
| `test-release-evidence.ps1` | 58 项通过 | 真实报告/清理函数；深度截断、写入失败和未知结果不能生成假通过 |
| `test-release-materials.ps1` | 通过 | 临时 Git fixture 验证独立材料来源、未提交/未推送/越界输入拒绝 |
| 既有 SSH Resume 块的一次性离线探针 | 10 项通过 | 真实检查块，设备调用替身；候选、工具、固定前缀、清理与残留映射门 |
| `test-regression.ps1 -Group policy` | 两项通过 | 公开源文件策略、diff 空白；不是完整软件门 |

原始输出保存在上述本地证据目录。JSON 深度测试中的两条 warning 是故意构造的
拒绝反例，不是正式报告被截断。离线 fixture 不证明新的真机恢复、性能收益或 C3 通过。
SSH 未运行任何实际认证组；发布材料测试未调用签名或发布。

### 工具交接与成本边界

独立工具任务已接手 idle-reset/命令完成观察的当前来源诊断，本批不重复修改输入链。
其 AT16-07 PUT/GET mapping 补丁只有离线证据，尚未验证目标 HDC 实际输出或采用；
当前没有该场景的实际需求，因此保留，不抢占原型设备验证。

本批不建立新的续跑引擎，不为已有功能重复开发。使用现有离线反例和一项最小 SSH
检查块探针替代整轮发布复跑；没有人工介入。未测得流程提速比例，不补造历史工时。

## 2026-09-16：命令完成观察链修复与定向真机验证

本批接收既有工具维护任务的八文件补丁，针对提交、完成观察和失败留存，不审计或
重写全部工具。源码反例证明提交 ACK 可能先于 `executeKeyCommand` 释放 pending；
known-host 投影原本不存在也不能证明完成。这不是旧 Wi-Fi 自然失败的完整归因。

验收注入现在观察真实 Session，在原 pending 释放后报告完成/失败/取消；消费者核对
Pane、代次和提交序号。缺失、冲突、取消或观察失败停止，不重发 Enter；业务失败的
fault domain 保持未知。原投影检查仍是独立后置条件，不增加业务 busy/idle 状态副本。
失败记录保存实际判定样本的白名单数值/枚举，不保存命令正文；先前分隔符脱敏漏洞
已修复，五个独立隐私反例通过。正常生产源不包含这些注入，发布包排除规则同步更新。

### 验证与结果

- 离线：12 个辅助函数、4 个实际 Mosh/SSH 调用者、8 个注入 owner 反例及样本留存、
  观察失败、取消、异常和还原检查通过；`policy,arkts` 聚焦检查含 202 项 ArkTS 测试通过。
- 首次真实编译发现新增 `catch/throw error` 不符合 ArkTS 的 `arkts-limited-throw`。
  改为在原 try/finally 记录 await 是否正常返回，保留原异常传播；异常对象身份反例、
  owner/生产隔离及源码还原复验通过。随后 ARM64 debug 构建、测试签名、安装和启动通过。
  JS 转译通过不代替 ArkTS 编译。
- 现有 `verify-ssh-auth-pc.ps1 -DiagnosticHap -Only password-success` 首次约 93.7 秒失败：
  中文输入法转换标点，期望/实际长度 31/30、首差异索引 10、非 ASCII 数 7。
  初始及 finally 输入均在 Enter 前被拒绝，Enter 合计为 0，未进入完成屏障。
  原清理报告保留失败；独立只读审计确认 known-host/映射无残留，fixture 已退出，
  常亮租约已恢复，不把补充审计改写成原场景通过。
- 截图确认“中→英”后，仅复验一次同包同场景，约 75.2 秒通过：3 条命令各一次输入、
  零不匹配，初始/最终 known-host 清理均收到匹配 owner 的完成信号；临时资源清理通过。
  原中文模式随后恢复，共享资源交还原型任务。未要求维护者操作。
- 第二次调用漏传已有 `-PreviousAttemptId`；原报告的空关联不修改。补充批次记录关联
  两次 attempt 和同包哈希，仅解释重试来源，不冒充原生续验或正式证据。

本地证据：`build/verification/development-workflow-20260916/`，包括失败/成功原报告、
编译日志、离线反例和 `command-completion-summary.json`。同一测试 HAP SHA-256 为
`475726e36bd8fd0b1f950a18f55a8e0f2f7c83d55dc0f46d33733ae04b4a9858`。
私有截图、布局和原始夹具资料不提交。无新性能收益测量，不推算节省比例。

### 剩余边界

输入法基线缺少提前确认属于非阻塞工具观察：本轮三次输入及 finally 又三次都遇到
相同转换，造成约 94 秒失败开销。当前低成本办法是在 ASCII fixture 前确认英文模式、
结束恢复原模式；不能凭非 ASCII 就自动归因环境。下次相关自动任务启动前评估是否
值得前移此检查，目前不新增切换框架、不改 xterm，也不扩大为历史丢字根因。
...
## 2026-09-16：ASCII 输入法前提的离线取舍

本记录对应 Next Work 的输入法前提项，不新增另一份执行清单。读取 PR #209 的
`command-completion-summary.json`：第一次 93715 ms 失败，期望/实际长度 31/30，
nonAsciiCount=7、Enter=0；明确英文模式后，同包一次 75179 ms 场景通过并恢复原中文模式。
这只证明该样本的输入法前提，不是全部历史丢字的根因或新方案的验收结果。

三份历史 UiTest 布局有 67、67、254 个节点，未找到精确的“中/英/中文/英文/English”
text 标签。不能据此推断所有设备都不可读取模式，但当前材料不足以实现可靠模式 getter。
已有 `Invoke-ImeInputProbe` 要求英文基线，按已知顺序切换；不能把它当作自动探测。
键事件返回成功或 toggle 次数也不能证明恢复了原模式。

本批选择复用显式观察与准备：记录原模式和目标焦点，必要时操作当前可辨认的控件，
再次确认英文，再由既有 native-buffer 精确检查决定是否允许 Enter。窗口或输入法变化
后重审；无法确认则停止。finally 在业务清理结束后、释放受控输入前恢复本场景拥有的
模式变更，并重新观察原模式。恢复不确定保留 unknown/failed，不修改旧失败报告。
具体执行合同见[质量策略](quality-strategy.md#ascii-fixture-input-mode-preparation)。

成本依据是已有约 94 秒失败开销；显式准备和恢复耗时未单独测量，不推算节省比例。
当前没有可靠的结构化模式观察入口，新增自动 getter/切换器还需实现、反例和真机资格，
不足以证明比既有显式准备更划算。因此本批只交付离线方案，不新增 helper、不改 xterm，
不使用旧坐标或盲按切换键。若下次场景暴露无法可靠执行的边界，再按新证据选择有限修理。
...
### 2026-09-22：PUT/GET 定向验证的前提与收尾

AT16-07 映射补丁的 9 项离线反例和真机枚举/移除通过。基础 PUT/GET 首轮在测试目录
准备时报 `Operation not permitted`：Downloads 授权关闭，零传输命令；目录、映射、
夹具进程和本地临时目录确认不存在。复用已有 `host-identity-downloads.ps1` 在诊断
wrapper 中准备权限并于 finally 恢复后，131,089 字节往返哈希一致、自动编号及原文件
保留通过。该 wrapper 为本地忽略的场景准备，不是产品适配或新的永久权限框架。

独立收尾发现旧基础模式只清理文件/映射/进程，未删除临时端口的主机信任。补充一次
产品 `ssh-keygen -R`，等待现有命令完成观察并用已有只读 helper 确认记录不存在，
所有自有资源和原权限最终恢复。传输报告只证明业务结果；完整收尾来自补充证据。
这不阻塞采用映射修复，也不能把该脚本单独晋升为完整自动清理或正式工具资格。
下次实际采用时明确准备/恢复/信任清理责任，优先复用既有步骤；本轮不扩展永久工具。
...
## 2026-09-24：1.7 正式矩阵的清理与旧文案观察边界

冻结工具 `06160c0` 的卸载场景未保存应用私有 `known_hosts`；1.7 已将主机信任从
持久资产迁入该文件，普通卸载会清空既有记录。此次由执行任务在独立 Host 场景
清理完成后、下一场景连接前保存原始 107 条记录，设备读前/读后及本地摘要相同，
不含本轮固定认证端点。矩阵结束后由同一任务恢复并校验原字节，补充结果单独记录，
不将场景自报 cleanup 扩称为所有既有数据已恢复。快照留桌面用户临时目录、用后
删除，不输出或提交主机身份；不顺便清除旧测试端口。下一轮实际采用卸载入口前
必须明确这项保存/恢复责任，可沿用人工步骤；本轮不新增通用备份框架或热改工具。

静态检查还发现 Mosh 合盖的进程替换分支仍搜索已删除的初始恢复 warning，而同进程
运行时回收的真实错误提示仍有效。当前自动阶段不经过前一分支。若人工阶段实际
触发，保留原工具结果，依据布局、日志和交互验证恢复，再按质量策略选受影响验证
及受支持续跑；不能手改报告或重新引入产品文案来满足旧断言。此处记录的是工具
判据风险，尚未触发的分支不记为通过或产品故障。

证据：`LeanTTY-release/formal-verification/1.7.0-06160c0-20260924/` 的
`preparation/trust-preservation.json` 和阶段报告；执行状态仅在 next-work 维护。

首轮随后在长任务前置清理停止：新装后文件合法地不存在，旧观察器却要求 `cat`
成功；Windows WSL 启动器结束后，精确归属该轮的 Linux sshd 仍监听 23150。
实际模型请求为零。独立清理后，PR #242 修正缺失文件观察并复用现有 Agent 场景的
Linux PID/配置归属及退出验证。49 项通知回归、九种真实 shell 文件状态、受影响
软件检查和零模型 shell 通知真机诊断通过，原 107 条信任按原摘要恢复。

旧 warning 判据也存在于受控进程重启入口。首轮停止后，PR #243 将两个进程替换
分支改为专用恢复日志与实际 Pane 布局，并保留旧输出消失和本地命令检查。首个
诊断因通用日志投影缺少状态所有者标签停止，清理通过；使用卸载场景已有的精确
PID/专用标签查询后，零模型重验通过。证据将布局恢复与 warning 可见性分开，
运行时会话回收仍保留其有效提示检查。两项改动均在原轮停止后合入并冻结，未热改
原工具或报告；下一轮选择 R3，复用原 C0–C2。前缀重跑约 33 分钟，没有重复模型
请求，比扩展报告导入/清理补证框架成本低。原始失败及修复证据分别保留在上述
正式目录和 `build/verification/trust-cleanup-20260924/`。

R3 的默认 ECDSA 在权限准备、导出前遇到另一已界定的导航限制：Settings 的当前
隐私页含一个侧栏标签和一个页标题，文字同为“隐私和安全”；全树唯一匹配拒绝
两个节点，清理通过，原权限关闭，密钥没有变更。当前任务采用人工准备/恢复该
场景需要的同一个 Downloads 开关，以 atm 与系统复选框核对，保持冻结工具不变，
用现有 R2 入口继续。不能把人工前提准备称为工具导航已修复。后续真正维护该导航
时应在页面标题所属节点内判断页面身份，并测试同名侧栏；不应删掉目标控件的
唯一性检查。证据在 R3 的 `host-identity-default-ecdsa/attempt-1/`，本轮采用和
恢复责任只在 next-work 维护，不在运行中扩展跨工具续跑框架。
