...
| 连接默认策略 | timeout、keepalive、取消 | 必须做 | 安全可靠默认；可配置子集进入 1.7 |
| 地址族 | `-4/-6`、`AddressFamily` | 应该做 | 1.7；强制实际网络族，不作为无效 flag |
| 安全诊断 | `-v` | 应该做 | 1.7；LeanTTY 结构化、脱敏诊断，不复制原始 OpenSSH log |
| 基本 escape | `~.`、`~?`、`~I` | 应该做 | 1.7；断开、帮助、连接信息；与 Pane 生命周期统一 |
| Jump Host | `ProxyJump`、`-J` | 应该做 | 1.5；配置为主，`-J` 复用同一状态机，首版单跳 |
...
| alternate config | `-F file` | 不做 | 受控 config import/export 进入 1.7，不建立临时第二权威 |
...
| timeout/keepalive | 应该做 | 1.7：`ConnectTimeout`、`ServerAliveInterval/CountMax` 的受控子集 |
| `AddressFamily` | 应该做 | 1.7；与 `-4/-6` 共用策略 |
| `UpdateHostKeys` | 应该做 | 1.7；须验证服务端扩展、原子持久化和错误恢复 |
| `ProxyJump` | 应该做 | 1.5；首版单跳，跳板与目标分别校验和认证 |
| config import/export | 应该做局部命令 | 1.7；通过 HarmonyOS 文件授权进入唯一 config，不用 `-F` |
...
| `-c` 修改 comment | 应该做 | 1.7；复用同一 key identity 和提交入口 |
| safe `-b/-C/-f` | 必须做受控子集 | 与生成 mode 一起保持明确范围 |
| ECDSA key 导入和认证 | 应该做，待实现证据 | 1.7 候选；不要求新增 ECDSA 生成入口 |
...
- 1.7 可增加受控 config import/export，但不得导入后静默忽略关键 directive。
