...
| 1.5 | `ConnectTimeout`、基本 SSH escape 与 `ServerAliveInterval/ServerAliveCountMax` 已闭合；`-4/-6` / `AddressFamily` 因真机 IPv6 fixture 门禁暂缓；当前只晋级 UpdateHostKeys 的标准、russh 能力和受控 fixture 基线，是否实现由证据另行决定。安全 `-v`、config import/export、`ssh-keygen -c`、ECDSA 导入/认证仍逐项晋级 |
