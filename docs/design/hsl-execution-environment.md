...
> 状态：Closed；两轮公开接口与物理机门禁均已完成，HSL 产品入口已裁剪
>
> 判定日期：2026-08-17
>
...
| HSL 专用使用指南 | HSL 当前只表现为普通 SSH 服务器 | 不提供；沿用现有 OpenSSH Host 与通用 SSH 文档 |

### 当前建议

HSL 项目现阶段已经闭合，不进入 discovery adapter、专用连接 UI 或专用指南开发。HSL 只作为
普通 SSH 服务器：用户提供当前 endpoint，LeanTTY 复用既有 Host、Identity、认证和主机校验。
这样不会把一次真机内部地址、包名或系统终端行为固化为长期维护负担。

第二轮真机证据已闭合所有可能改变门禁的公开发现方向，没有出现重新进入产品实现的条件。
不再为 HSL 建立单独的产品或文档路径；只有平台未来提供满足“重新进入条件”的公开稳定接口，
才重新讨论专用入口。普通 SSH 使用继续由现有通用文档覆盖。
