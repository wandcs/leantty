...
>
> 后续变更仍以 [`next-work.md`](../next-work.md) 为唯一有效 TODO。本文保留产品边界、
> 实现约束和已闭合的验收基线，不维护第二份活动清单。
>
...
2026-08-08，1.2.0 提交 AppGallery 审核后，用户确认开始 1.3 开发。到 2026-08-14，受约束
`put/get`、本地无覆盖提交、SFTP/OpenSSH 互操作、FD/no-follow、生命周期、路径/补全、进度、
取消、断线、冲突和失败清理均已实现，并通过自动化、干净 ARM64 构建与适用的物理机矩阵。
首次未授权状态矩阵按本文记录的产品取舍默认跳过，不再阻塞发布。候选冻结、剩余核心终端人工
回归和正式发布步骤仍只由 `next-work.md` 管理；构建或安装成功本身不等于行为验证。
...
基于上述证据，2026-08-14 决定将首次未授权、拒绝恢复和双 Pane 授权 single-flight 从 1.3
发布门禁中移除，后续版本默认跳过。生产实现继续保留现有单一 `activeRequest` 所有权和失败
恢复路径，相关自动化继续作为常规回归；但不为单次难以安全复现的系统前置状态破坏实体机数据
...
以下条目是 1.3 收口时使用的自动化与物理机验收范围，不是当前 TODO。完成事实以
`CHANGELOG.md`、合并 PR 和不可变 1.3 发布身份为准；后续只有相关事件链发生变化时，
才按 `quality-strategy.md` 选择受影响的回归场景。
...
8. 中途无选区 `Ctrl+C`、有选区只复制、确认关闭 Pane、确认关闭应用、断网、最小化和
   suspend/wake 均沿 Pane 所有权与任务清理边界完成验收。
...
1.3 在自动化、ARM64 构建、上述适用真机矩阵和正式发布身份共同闭合后标记为 Verified；
后续代码或测试通过不能反向改变这一历史发布事实，也不能自动证明新版本仍满足相同门禁。
...
| 验证是设计的一部分 | 自动化、ARM64 构建和命名物理机矩阵分别闭合策略、集成与真实平台行为；后续变更按事件链选择回归 |
...
1.3 已按上述边界发布。后续若重新设计或扩大文件传输，出现以下任一情况时应停止扩展，
而不是用更多功能补救：
