...
## 2026-09-12 Mosh 命令跨阶段 owner 与旧证据审计

### 判定修复

在 `d479e8b` 上执行实际 `Focus-ActiveTerminalInput`、`Submit-MoshInput` 和共享注入
函数，仅替换设备/PTY I/O。确定性反例证明两个问题：同一原生 Web 在 Enter 前发生祖先
重排会被误拒绝；不同 Web 复用同一虚拟路径反而会获准 Enter。稳定布局对照通过。
最后正确边界是共享 post-input 检查，首个错误边界是 Mosh 随后的无布局字段比较。
这证明工具合同缺陷，不证明真机上曾发生串 Pane 或产品丢字。

本任务开始时重新核对 [OpenHarmony 6.1 原生节点源码](https://github.com/openharmony/testfwk_arkxtest/blob/1fc51561281f02fda42e65ec952fb21406b05981/uitest/server/element_node_iterator_impl.cpp)：
原生 hashcode 使用 window/accessibility ID，导出的原生属性取自同一对象。公开源码不是
测试机厂商二进制的逐字证明；此前设备基线为 OpenHarmony 6.1.1.135 / UiTest 6.0.2.3。
上游 issues、华为社区和公开复现检索未找到相同 Web 重排报告。
[Playwright 的唯一定位规则](https://playwright.dev/docs/locators#strictness) 只用作设计参照。

修复让 Mosh 保存本次焦点捕获的 node/layout，并在输入前、重试前和 Enter 前复用同一个
共享原生 owner 判定。重新聚焦时使用实际返回的布局和节点，拒绝 Web 替换及多重焦点。
共享注入接受可选 `InputLayout`，Mosh 使用它保留第一次输入前的身份。
没有新增设备读取、延时、重试或持久缓存；远端精确输入先于单次 Enter，原三次不匹配
上限和清空确认不变。

直接调用边界已核对：Mosh 三个跨捕获比较使用完整布局；共享注入的 post-input 仍使用
完整布局。其他仅提供节点的调用方保持即时字段预检，不宣称它们具有跨命令、导航或
重建的原生实例身份合同。本轮没有该边界的自然失败，不为潜在竞态扩展全部焦点工具。

新增 22 项真实提交链用例和 4 项聚焦用例。祖先重排在输入前的用例先在旧实现失败，
修复后全部通过，覆盖虚拟重建、不同/缺失/重复 owner、窗口、多焦点、重试保留、
重试耗尽、清空失败及意外返回本地提示符。普通成功路径仍为四次现有布局捕获、
一次文本注入、一次 Enter。policy/tooling 七项组合检查通过；首次组合在沙箱内运行，
Agent 辅助检查因 WSL 身份访问拒绝停止，桌面用户重跑通过，原失败证据保留。

干净诊断 harness：`7039f8a6d110889a433919f7752925241ea113ef`，
tree `74ffa5227811fbbe89e0e0bd99ec9aed7b75f126`。原签名 HAP
`0db0f090cc9053b15204d55f6a49a2df202aa970eaf3e3672192fb668da95e71`
的 `fixed-endpoint` 诊断于 01:11–01:14（UTC+8）通过，耗时 160302 ms：
受控远端命令一次输入、零不匹配、一次 Enter；认证关闭、原页面恢复、临时页面丢弃、
偏好不变和清理均通过。独立只读审计确认临时 Host、known-host、HDC 映射、TCP/UDP
监听、夹具进程和目录缺席，工作区仍为一 Tab。未重建 HAP、调用模型、改变网络、
要求人工操作或执行 QH/完整 C3。

证据根：`build/verification/input-contract-resume-20260912/`。
`mosh-owner-before.json` 保留三项原行为；`software-focused.json` 保留身份失败，
`software-desktop.json` 是完整组合通过结果；`mosh-fixed-endpoint/device-mosh.json`
SHA-256 为 `b8496790ebba55d4a36ac0ce0f24b97dc9b8591906580ff857ec0d886edef080`；
`closing-audit.json` 是独立收尾审计。这些诊断不替代正式验收，也不声称重放自然竞态。

### 旧结果能继承到哪里

只读审计 PR #179 的原正式报告及八项 Mosh 子报告，保留原哈希，不修改结果。
24 条受控 Mosh 命令均记录单次输入、零不匹配和单次 Enter。八个场景各保留最后一对
焦点布局，全部通过当前原生 owner 判定；但文件名只含输入尝试序号，跨命令覆盖。
除单命令 runtime-reclaim 外，其余场景没有完整命令布局对，共享注入的临时布局也已
按合同删除。不能用最后一对布局证明全部历史命令符合新检查。

C0–C2 和先前观察到的锁屏、合盖、Wi-Fi 恢复事实保留；旧通过事实不被改写为失败。
新 harness 的 QH 必须重新取得。旧 Agent 失败和未执行的 SSH 不能复用。
完整 C3 目前不满足跨 harness 继承前提：新反例说明旧检查不仅可能误拒绝，也可能
放行替换目标，而历史数据不能对所有调用重放新判定。

比较结论：仅放宽提交哈希限制不能补回证据；新增可审计继承机制还需处理来源哈希、
阶段身份、失效范围及嵌套矩阵，仍不能解决本次缺失的输入布局。重跑的已观察成本约
