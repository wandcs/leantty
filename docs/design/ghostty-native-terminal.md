...
opacity 会破坏既有语义。[产品原则 §4.7–4.8](../project-principles.md) 的 WebGL/DOM
表述在正式替换方案冻结时需要更新；本轮只评估新实现，可靠恢复的要求继续成立。

GPU 失败的候选行为：停止使用旧 Surface/context，保留 VT 与 Session，封闭旧帧回调，
重建 GPU 资源后全量重画。重建持续失败时，显示明确的本地故障状态并阻止向不可见终端盲输。
永久失败下如何保持既有“终端可用”承诺仍未闭合，不能把报错当成恢复成功或默许黑屏。
此问题阻止正式替换；目前不为它提前建设 CPU renderer。
...
引入 HarfBuzz、FreeType、`libghostty-ohos` 或另一套 renderer 的技术缺口。原先要求立即
决定持续 GPU 故障合同的建议，经下文追加诊断调整为先完善恢复链；正式替换的可用性
要求不变，不能由实验通过数代替。
...
当时提出但未获接受的方案是维持单一 GLES 实现：失败时有限自动重建，仍失败则显示原生故障页，保留仍存活的
Session/VT、暂停终端输入，允许重试和显式关闭；GPU 恢复前无法继续终端交互。不要因
这个假设场景直接增加 CPU renderer，也不要让陈旧最后一帧看起来仍可输入。原型已经
验证提示、状态保留和随后恢复；输入冻结、重试入口与真实 Session 行为留待产品实现。

这会改变[产品原则 §4.8](../project-principles.md#48-终端-renderer-选择) 当前的
“恢复路径必须保持终端可用”承诺。用户已选 GPU 路线，但尚未明确接受持续故障时中断
交互，故不能自行改原则或把故障页当可用性通过。上述原提案尚未获接受；维护者随后要求
先分析并解决 GPU 问题。追加诊断发现原型自己的恢复缺口，已修复下列可恢复场景，因此
不再把这个假设性取舍作为当前接入前置问题。正式删除旧 renderer 前仍须闭合既有可用性
条件；若可靠恢复完成后仍有无法处理的实际失败，再基于具体证据讨论残余合同。
